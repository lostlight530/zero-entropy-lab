...
适用边界: 受控确定性任务, 覆盖事实源代际、非幂等意图、共享依赖上下文与半开故障域.
...
独立性: 五项实验分布在五个时间窗口, 分别改变事实源、非幂等意图、共同故障、分区恢复和租约接管. 强反例覆盖旧事实源自洽、同键不同意图、聚合恢复误导、全局熔断过早关闭与过期持有者覆盖.

限制: 尚未覆盖真实并发租约、续约确认丢失、时钟漂移、不可逆远端副作用和验证器完全独立实现. 零副作用是本组受控重放结果, 不是所有重试都应跳过的通用规则.
