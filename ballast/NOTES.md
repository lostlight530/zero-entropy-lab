...
独立性: 六项实验分布在六个时间窗口, 分别改变事实源、非幂等意图、去重保留期、共同故障、分区恢复和租约接管. 强反例覆盖旧事实源自洽、同键不同意图、同标识不同资源、聚合恢复误导、全局熔断过早关闭与过期持有者覆盖.

限制: 尚未覆盖真实并发租约、续约确认丢失、时钟漂移、远端索引延迟、不可逆副作用和验证器完全独立实现. 零副作用是本组受控重放结果, 不是所有重试都应跳过的通用规则.
