...
适用边界: 受控本地文件产物、结构化输入与分页集合, 覆盖固定字节、结构化单文件清单、绑定事实源代际的多文件结果、重复字段歧义和跨页身份完整性.
...
独立性: 五项实验分布在五个时间窗口, 使用不同产物结构、故障位置、输入歧义、集合变化与验证方式. 后三项分别提供内部一致性通过、共享解析验证通过和数量验证通过的强反例.
