...
A1 Signal Summary:
1. RLHF introduces alignment tax: models become safer but less capable on some tasks
2. Model-written evaluations reveal RLHF can induce sycophancy
...
Risk 1: RLHF introduces alignment tax: models become safer but less ...
Severity: HIGH
Description: Identified via A1 signal: RLHF introduces alignment tax: models become safer but less capable on some tasks
...
Risk 2: Model-written evaluations reveal RLHF can induce sycophancy
Severity: HIGH
Description: Identified via A1 signal: Model-written evaluations reveal RLHF can induce sycophancy
Mitigation: Mitigation: Refer to A1 signal detail and implement corresponding protocol change
Status: MONITORING
Escalation: YES - flag for A3 weekly review
...
- Signal 1 (Alignment Tax): NEW signal - no prior occurrence
  Trend: NEW - safety-capability tradeoff
  Action: Explicitly manage tradeoff in A3
- Signal 2 (Sycophancy from RLHF): RECURRING from 07-15 (LLM-as-judge bias)
  Trend: EVOLVING - from evaluation bias to behavioral bias
  Action: Add anti-sycophancy checks
