...
- **Daily Coverage Matrix**: 11 files present
...
- **历史 A4**: aegis-cortex/2026-W36-A4-protocol-act.md
- **A6**: aegis-cortex/2026-08-A6-aegis-memorize.md
- **新鲜度来源**: 联网检查 Crossref API 查询，确认学术研究持续存在无失效
- **失效决策**: NONE
...
- **缺失输入处理**: 若在聚合时遇到类似 W37 的 INPUT_MISSING，如实映射为空白输入并降低该时段置信度，严禁在未读取的前提下脑补风险。
- **需要继续验证的问题**: 复杂的多步任务是否能在不引入原生代码检查点的情况下安全执行。
- **失效条件**: 如果没有触发相应停用条件或月度/周度刷新，这些纪律自动过期。
